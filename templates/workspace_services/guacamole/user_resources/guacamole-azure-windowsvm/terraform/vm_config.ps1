if( ${SharedStorageAccess} -eq 1 )
{
  ...
  net use y: \\${StorageAccountName}.file.core.windows.net\ingress /u:AZURE\${StorageAccountName} ${StorageAccountKey}
  net use x: \\${StorageAccountName}.file.core.windows.net\egress /u:AZURE\${StorageAccountName} ${StorageAccountKey}"
